...
Mongodb.prototype.save = function(callback) {
    ...
    queue = async.queue(function (document, callback) {
        self._collection.save(document, {safe:seslf.safe_collections}, function(err) {
            if (err) console.error(err);
        });
        callback();
    }, 4);
